...
*/
#define SYNTH_URI "http://lv2plug.in/plugins/synth"
...
connect_port(LV2_Handle instance,
             ...
             void*      data)
{
	Synth* s = (Synth*)instance;

	switch ((PortIndex)port) {
	case SYNTH_GAIN:
		s->gain = (const float*)data;
		break;
	case SYNTH_INPUT:
		s->input = (const float*)data;
		break;
	case SYNTH_OUTPUT:
		s->output = (float*)data;
		break;
...
{
	const Synth* s = (const Synth*)instance;

	const float        gain   = *(s->gain);
	const float* const input  = s->input;
	float* const       output = s->output;
